...
# By default use -O0 for debug and -O2 for release
set(OPT_OPTIMIZATION $<IF:$<CONFIG:Debug>,-O0,-O2>)
...
elseif(${OPT_BOARD} STREQUAL stm32f207zg_ethboard_v2)
    set(ARCH cortexM3_stm32f2)
elseif(${OPT_BOARD} STREQUAL stm32f207zg_nucleo)
    set(ARCH cortexM3_stm32f2)
...
elseif(${OPT_BOARD} STREQUAL stm32h753xi_eval)
    ...
    message(FATAL_ERROR "${OPT_BOARD} board not found in miosix/config/options.cmake")
endif()
...
set(CXXFLAGS_BASE "-D_MIOSIX_BOARDNAME=\"${OPT_BOARD}\"" -D_DEFAULT_SOURCE=1 -std=c++14 -ffunction-sections -Wall -Werror=return-type -g)
...
## Then for each architecture there is a switch for every board, where all
...
if(${ARCH} STREQUAL arm7_lpc2000)
    ...
    if(${OPT_BOARD} STREQUAL lpc2138_miosix_board)
        ...
        set(ARCH_SRC
            ...
            ${KPATH}/${BOARD_INC}/interfaces-impl/os_timer.cpp
...
elseif(${ARCH} STREQUAL cortexM3_efm32gg)
    ...
    if(${OPT_BOARD} STREQUAL efm32gg332f1024_wandstem)
        ...
        set(ARCH_SRC
            ...
            ${KPATH}/${BOARD_INC}/interfaces-impl/os_timer.cpp
            ${KPATH}/${BOARD_INC}/interfaces-impl/timer_interface.cpp
            ${KPATH}/${BOARD_INC}/interfaces-impl/rtc.cpp
            ${KPATH}/${BOARD_INC}/interfaces-impl/hrtb.cpp
            ${KPATH}/${BOARD_INC}/interfaces-impl/gpio_timer.cpp
            ${KPATH}/${BOARD_INC}/interfaces-impl/transceiver_timer.cpp
            ${KPATH}/${BOARD_INC}/interfaces-impl/gpioirq.cpp
            ${KPATH}/${BOARD_INC}/interfaces-impl/transceiver.cpp
            ${KPATH}/${BOARD_INC}/interfaces-impl/flopsync_vht.cpp
            ${KPATH}/${BOARD_INC}/interfaces-impl/vht.cpp
            ${KPATH}/${BOARD_INC}/interfaces-impl/virtual_clock.cpp
        )
...
elseif(${ARCH} STREQUAL cortexM3_stm32f1)
    ...
    ##
    if(${OPT_BOARD} STREQUAL stm32f100c8_microboard)
    ...
    elseif(${OPT_BOARD} STREQUAL stm32f103cb_als_mainboard_rev2)
        ...
        ## systems with the binary or hex file repectively.
        ## If a command is not specified, the build system will use st-flash if found
        set(PROGRAM_CMDLINE "stm32flash -w $binary -v /dev/ttyUSB1")
    ...
    elseif(${OPT_BOARD} STREQUAL stm32f103ve_mp3v2)
        ...
        #set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_512k+64k_ram.ld)
        ...
        ## systems with the binary or hex file repectively.
        ## If a command is not specified, the build system will use st-flash if found
        if(${LINKER_SCRIPT} STREQUAL ${KPATH}/${BOARD_INC}/stm32_512k+64k_ram.ld)
            set(PROGRAM_CMDLINE "mp3v2_bootloader --ram $binary")
        else()
            set(PROGRAM_CMDLINE "mp3v2_bootloader --code $binary")
        endif()
    ...
    elseif(${OPT_BOARD} STREQUAL stm32f103ve_strive_mini)
        ...
        ## systems with the binary or hex file repectively.
        ## If a command is not specified, the build system will use st-flash if found
        set(PROGRAM_CMDLINE "c:/Program Files/STMicroelectronics/STM32 ST-LINK Utility/ST-LINK Utility/ST-LINK_CLI.exe -c JTAG -Rst -P $hex 0x08000000 -Run")
    ...
    elseif(${OPT_BOARD} STREQUAL stm32f103ze_redbull_v2)
        ...
        ## systems with the binary or hex file repectively.
        ## If a command is not specified, the build system will use st-flash if found
        set(PROGRAM_CMDLINE "C:/Program Files/SEGGER/JLinkARM_V434d/JFlashARM.exe -openprjSTM32F10xxE.jflash -open$hex -auto -exit")
        #set(PROGRAM_CMDLINE "c:/Program Files/STMicroelectronics/STM32 ST-LINK Utility/ST-LINK Utility/ST-LINK_CLI.exe -c JTAG -Rst -P $hex 0x08000000 -Run")
    ...
    elseif(${OPT_BOARD} STREQUAL stm32f103ze_stm3210e-eval)
        ...
        #set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_512k+64k_rom.ld)
        #set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_512k+64k_xram.ld)
        set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_512k+64k_all_in_xram.ld)
...
elseif(${ARCH} STREQUAL cortexM3_stm32f2)
    ...
    ##
    if(${OPT_BOARD} STREQUAL stm32f205_generic)
    ...
    elseif(${OPT_BOARD} STREQUAL stm32f207ig_stm3220g-eval)
        ...
        ## (see the XRAM flag below)
        #set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_1m+128k_rom.ld)
        #set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_1m+128k_xram.ld)
        set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_1m+128k_all_in_xram.ld)
        #set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_1m+128k_all_in_xram_processes.ld)
        ...
        ## systems with the binary or hex file repectively.
        ## If a command is not specified, the build system will use st-flash if found
        if(${LINKER_SCRIPT} STREQUAL ${KPATH}/${BOARD_INC}/stm32_1m+128k_all_in_xram.ld)
            set(PROGRAM_CMDLINE "${KPATH}/_tools/bootloaders/stm32/pc_loader/pc_loader /dev/ttyUSB0 $binary")
        else()
    ...
    elseif(${OPT_BOARD} STREQUAL stm32f207zg_ethboard_v2)
        ...
        #set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_1m+128k_rom.ld)
        set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_1m+128k_code_in_xram.ld)
        ...
        ## systems with the binary or hex file repectively.
        ## If a command is not specified, the build system will use st-flash if found
        if(${LINKER_SCRIPT} STREQUAL ${KPATH}/${BOARD_INC}/stm32_1m+128k_code_in_xram.ld)
    ...
    elseif(${OPT_BOARD} STREQUAL stm32f207zg_nucleo)
        ...
        ## Base directory with header files for this board
        set(BOARD_INC arch/cortexM3_stm32f2/stm32f207zg_nucleo)
        ...
        ## Add a #define to allow querying board name
        list(APPEND CFLAGS_BASE -D_BOARD_STM32F207ZG_NUCLEO)
        list(APPEND CXXFLAGS_BASE -D_BOARD_STM32F207ZG_NUCLEO)

        ## Clock frequency
        ## External clock is 8Mhz provided from MCO output of ST-LINK. Actual
        ## HSE crystal (X3) is not fitted.
        set(CLOCK_FREQ -DHSE_VALUE=8000000)
        set(CLOCK_FREQ -DSYSCLK_FREQ_120MHz=120000000)
...
elseif(${ARCH} STREQUAL cortexM3_stm32l1)
    ...
    ##
    if(${OPT_BOARD} STREQUAL stm32l151_als_mainboard)
...
elseif(${ARCH} STREQUAL cortexM4_atsam4l)
    ...
    set(AFLAGS_BASE -mcpu=cortex-m3 -mthumb)
    list(APPEND CFLAGS_BASE -D_ARCH_CORTEXM4_ATSAM4L -mcpu=cortex-m3 -mthumb ${OPT_OPTIMIZATION} -c)
    list(APPEND CXXFLAGS_BASE -D_ARCH_CORTEXM4_ATSAM4L -mcpu=cortex-m3 -mthumb ${OPT_EXCEPT} ${OPT_OPTIMIZATION} -c)
    set(LFLAGS_BASE -mcpu=cortex-m3 -mthumb -Wl,--gc-sections,-Map,main.map -Wl,-T${LINKER_SCRIPT} ${OPT_EXCEPT} ${OPT_OPTIMIZATION} -nostdlib)
    ...
    list(APPEND ARCH_SRC
        ...
        ${KPATH}/${ARCH_INC}/interfaces-impl/portability.cpp
        ${KPATH}/${ARCH_INC}/interfaces-impl/os_timer.cpp
        ${KPATH}/${ARCH_INC}/interfaces-impl/gpio_impl.cpp
        ${KPATH}/${ARCH_INC}/interfaces-impl/delays.cpp
    )
...
elseif(${ARCH} STREQUAL cortexM4_stm32f4)
    ...
    ##
    if(${OPT_BOARD} STREQUAL stm32f401re_nucleo)
    ...
    elseif(${OPT_BOARD} STREQUAL stm32f407vg_stm32f4discovery)
        ...
        ##    to configure the kernel with "#define WITH_PROCESSES"
        set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_1m+192k_rom.ld)
        #set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_1m+192k_ram.ld)
        #set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_1m+192k_rom_processes.ld)
        ...
        set(ARCH_SRC
            ...
            ${KPATH}/${BOARD_INC}/interfaces-impl/deep_sleep.cpp
            ${KPATH}/${BOARD_INC}/interfaces-impl/bsp.cpp
        )
    ...
    elseif(${OPT_BOARD} STREQUAL stm32f429zi_oledboard2)
        ...
        #set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_2m+256k_rom.ld)
        #set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_2m+8m_xram.ld)
        set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_2m+6m_xram.ld)
    ...
    elseif(${OPT_BOARD} STREQUAL stm32f429zi_skyward_homeone)
        ...
        ## systems with the binary or hex file repectively.
        ## If a command is not specified, the build system will use st-flash if found
        # set(PROGRAM_CMDLINE "here your custom flash command")s
    ...
    elseif(${OPT_BOARD} STREQUAL stm32f469ni_stm32f469i-disco)
        ...
        set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_2m+384k_rom.ld)
        #set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_2m+16m_xram.ld)
        #set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_2m+12m_xram.ld)
...
elseif(${ARCH} STREQUAL cortexM7_stm32f7)
    ...
    elseif(${OPT_BOARD} STREQUAL stm32f767zi_nucleo)
        ...
        ## Base directory with header files for this board
        set(BOARD_INC arch/cortexM7_stm32f7/stm32f767zi_nucleo)
...
elseif(${ARCH} STREQUAL cortexM7_stm32h7)
    ...
    if(${OPT_BOARD} STREQUAL stm32h753xi_eval)
        ...
        ##    You must uncomment -D__ENABLE_XRAM below in this case.
        set(LINKER_SCRIPT ${KPATH}/${BOARD_INC}/stm32_2m+512k_rom.ld)
